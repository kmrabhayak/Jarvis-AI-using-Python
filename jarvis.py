...
def wishMe():
    playsound.playsound( "D:\Program Files\Python37\JarvisBoot\HUD Activation Sound Effect.mp3" )
    playsound.playsound( "D:\Program Files\Python37\JarvisBoot\FridayBoot.mp3" )
...
wakeWords = ["hey jarvis", "ok jarvis", "you there jarvis", "wake up jarvis", "hai jarvis", "jarvis are you up",
             "you up jarvis"]
...
searchGoogle = ["search google", "search in google"," find in google"]
findLocation = ["find location", "find this location", "locate a place", "find a location", "locate this place",
                "find this place"]
months = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November",
          "December"]
weatherCheck = ["get weather", "what is the weather", "check weather", "weather information", "wetaher details",
                "how is the weather"]
yesWords = ["yeah", "ok", "yes", "go on", "fine", "yup", "go for it"]
noWords = ["no", "no need", "naco", "nope", "nop", "not now"]
musicWords = ["sing a song", "play music","play song" "music jarvis", "i am feeling groovy"]
...
def GetWeather(city="kochi"):
    try:
        wquery = "q=" + city
        res = requests.get(
            'http://api.openweathermap.org/data/2.5/weather?' + wquery + '&APPID=6efd2ff32715ec6f8058f256c6535fec&units=metric' )
        result = res.json()
        tempStr = "The temperature in " + city + " as of now is " + str( result["main"]["temp"] ) + " degree celsius"
        tempStr2 = "But it may vary between " + str( result["main"]["temp_min"] ) + " degree celsius" + " and " + str(
            result["main"]["temp_max"] ) + " degree celsius"
    ...
    while True:
        ...
        for phrase in shutdownWords:
            if phrase in query:
                ...
                playsound.playsound(
                    "D:\Program Files\Python37\JarvisBoot\system shut down sound FX.mp3" )
